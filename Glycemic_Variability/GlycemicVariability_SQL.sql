...
--81. Display the patient with rount value of hrv 
select patientid, ceil(avg(rmssd_ms*600)) as HRV from public.ibi group by patientid

--82, Display the patient who consume more carbs a day.

Select patientid,floor(sum(total_carb)) as totalcarbs ,extract(day from datetime) as per_day
from public.foodlog group by patientid,extract(day from datetime) order by totalcarbs desc limit 1

--83. Display the datestamp  glucose_value_mgdl and eda test was done

select patientid,datestamp from public.dexcom union select patientid,datestamp from public.eda

--84. split date and time in two different column in date time of foodlog
  SELECT patientid, logged_food,
    split_part(datetime::TEXT,'-', 1) y,
    split_part(datetime::TEXT,'-', 2) m,
	 split_part(datetime::TEXT,'-', 3) d
    FROM public.foodlog
  
-- 85. create index on table eda

CREATE INDEX index_name
ON  public.eda (patientid,max_eda);

--86. trim the fruit from the smoothies
SELECT
    TRIM('Fruit' FROM logged_food) as trimmed_smoothie from 
   public.foodlog where logged_food like '%smoothie%'

--87. Rank the eda table based on eda type 
-- Row number function Assigns a unique number to each row without gaps, regardless of equal values.
SELECT
    id,
    event_type,event_subtype,
    ROW_NUMBER() OVER (ORDER BY glucose_value_mgdl) AS ranking
FROM public.eventtype group by event_type,id;


--88. spliting the string Glycemic variablity 

SELECT  SUBSTRING('Glycemic variablity' FROM 1 FOR 8) AS result

--89. print 0 if the column has null

SELECT patientid, COALESCE(glucose_value_mgdl, '0') AS non_null_column
FROM public.dexcom;


--90. list the patient names in capital letter

select patientid, upper(firstname)||' '|| upper(lastname) as fullname from public.demographics

--91.Display the current date and time 

	select NOW()
	
--92. write a function to display the given mean hr category 
...
RAISE NOTICE 'high';
END IF;
IF mean_hr between 60 and 100 THEN
RAISE NOTICE 'normal';
END IF;
END $$;


--93. Display patientid and firstname in same row

SELECT CONCAT(patientid, REPEAT(':', 2), ' ', firstname) as patientid_hr
FROM public.demographics;

--94. change the event type of eventid 10 to EGV
SELECT TRANSLATE(event_type, 'Alter', 'EGV') from public.eventtype where id=10;


--95.Display patients SOME and ANY comparison conditions are similar to each other and are completely interchangeable.

select *
from public.demographics
where extract(year from "dob") =some(select extract(year from "dob")
from public.demographics
where gender='FEMALE');

--96. display the patients who healthy based on hr and A1c 

select patientid from public.hr where mean_hr between 60 and 100 and patientid in
(select patientid from public.demographics where hba1c<5.7)group by patientid
